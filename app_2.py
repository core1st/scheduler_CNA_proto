...
with st.sidebar.expander("➕ 새 기재(Row) 추가하기"):
...
raw_list = list(set(base_resources + existing_from_excel + custom_added))

# [핵심 1] Python Natural Sort (숫자 인식 정렬)
def natural_sort_key(s):
    return [int(text) if text.isdigit() else text.lower() for text in re.split(r'(\d+)', str(s))]

all_resources = sorted(raw_list, key=natural_sort_key)
...
with st.expander("📊 데이터 테이블 보기"):
    ...
    # Resource 컬럼을 Categorical로 변환하여 정렬 순서 강제 적용
    df_display['Resource'] = pd.Categorical(df_display['Resource'], categories=all_resources, ordered=True)
    st.dataframe(df_display.sort_values('Resource'))

# --- 7. JSON 변환 ---
# [핵심 2] groups 데이터에 'order' 필드를 추가하여 Python 정렬 순서(Index)를 주입함
...
for i, row in df_combined.iterrows():
    ...
    items.append({
        "id": i, "group": row['Resource'], "content": row['Label'],
...
<script>
  try {{
      ...
      var options = {{
        // [핵심 3] groupOrder를 'content'(이름순)가 아니라 'order'(번호순)으로 변경!
        groupOrder: 'order', 
      ...
      function exportData() {{
        ...
        navigator.clipboard.writeText(JSON.stringify(simpl)).then(function() {{
            document.getElementById('msg').innerText = "복사 완료!";
        }});
      }}
  }} catch (err) {{
...
st.subheader("4. 인터랙티브 스케줄러")
components.html(html_code, height=650)

# --- 9. 저장 ---
st.markdown("---")
st.subheader("5. 결과 저장")
json_input = st.text_area("복사한 데이터 붙여넣기", height=100)
...
if json_input:
    try:
        ...
        st.download_button("📥 엑셀 다운로드", to_excel(new_df), 'schedule_final.xlsx')
